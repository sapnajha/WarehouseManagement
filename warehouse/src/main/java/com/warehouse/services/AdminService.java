...
public class AdminService {
	...
	public List<Item> viewItems() {
		List<Item> item = itemRepository.itemList();
		return item;
	}
	...
	public String addStock(Item item) {
		List<Item> itemlist = itemRepository.itemDetails(item.getItem_code());
		int stock = itemlist.get(0).getItem_stock();
		stock += item.getItem_stock();
		item.setItem_stock(stock);
		itemRepository.updateItem(item.getItem_stock(), item.getItem_code());
		return "success";
	}
	...
	public String placeOrder(String customer_name, int item_code, int item_stock) {
		...
		purchase.setCustomer(customerRepository.findById(customer_name).get());
	...
	public Optional<Customer> view_A_Customer(String customer_name) {
		Optional<Customer> customer_data = customerRepository.findById(customer_name);
